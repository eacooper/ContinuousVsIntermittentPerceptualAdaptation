...
plot(x_coords(1:2),nanmean(Disp(:,1:2,Sind)),'-','Color',color_01,'LineWidth',2);
plot(x_coords(2:3),nanmean(Disp(:,2:3,Sind)),'-','Color',color_02,'LineWidth',2);
plot(x_coords([1 4]),nanmean(Disp(:,[1 4],Sind)),'-','Color',color_03,'LineWidth',2);

%plot each trial
data = scatter(repelem(x_coords,30), reshape(Disp(:,:,Sind),1,numel(Disp(:,:,Sind))),24,'^','MarkerEdgeColor',color_lightgray);

% plot a marker for each average
Avg = plot(x_coords, nanmean(Disp(:,:,Sind)),'k^','MarkerFaceColor','k','MarkerEdgeColor','k'); Avg(1).MarkerSize=8; 
...
set(gca,'plotboxaspectratio',[1.3 1 1],'box','off','FontSize',11);
...
scatter([0.40,2.1,3.8],[Disp_Dif_Start_Pre,Disp_Dif_End_Start,Disp_Dif_Post_Pre],60,'o','MarkerFaceColor','k','MarkerEdgeColor','k');
