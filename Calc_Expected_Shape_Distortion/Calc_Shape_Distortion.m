...
% calculates the ratio of the two sides of the rectangle.

clear all;
close all;

% PARAMETER SET UP
% distance from cyclopian eye to the rectangle (meters)
dist_m = 0.293;

% size of rectangle
RectH_dg    = 16;  % in visual degrees
RectW_dg    = 16;
RectHalfW_m = (2*(dist_m )).*tand((RectW_dg./2)./2);  % convert to half width in meters
...
% Corners of a rectangle in meters
...
% Identify the x position of the corners given the slant
...
or = plot3(x(:),y(:),z(:),'-ko'); %original dots
view([0 90]); %side view
xlabel('x'); ylabel('y');
...
figure(51); hold on; title('perspective projection');
...
%this is the value that the red arrow signifies in figure 8 which is a difference in ratios between the start minus pre measurement
